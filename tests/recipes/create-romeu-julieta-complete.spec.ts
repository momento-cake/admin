...
test.describe('Recipe Creation - Romeu e Julieta Complete', () => {
  test('should create complete Romeu e Julieta Tradicional recipe', async ({ page }) => {
    ...
    // Fill quantity for first ingredient (find the input with value "1")
    ...
    // Fill quantity for second ingredient (find the input in the second ingredient row)
    const secondIngredientRow = page.locator('div').filter({ hasText: 'Goiabada' }).first();
    await secondIngredientRow.locator('input[value="1"]').fill('200');
    ...
    await page.fill('input[placeholder="Duração em minutos"]', '5');
    ...
    const secondStepDuration = page.locator('input[placeholder="Duração em minutos"]').nth(1);
    ...
    const thirdStepDuration = page.locator('input[placeholder="Duração em minutos"]').nth(2);
    ...
    const fourthStepDuration = page.locator('input[placeholder="Duração em minutos"]').nth(3);
    await fourthStepDescription.fill('Sirva em temperatura ambiente');
    await fourthStepDuration.fill('0');

    // Add notes
    await page.fill('textarea[placeholder="Observações sobre a receita"]', 'Combinação tradicional brasileira. Use queijo minas bem fresco e goiabada de boa qualidade. Pode ser servido como sobremesa ou lanche.');

    // Take screenshot before saving
    await page.screenshot({ 
      path: 'tests/screenshots/romeu-julieta-complete-form.png',
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: 'tests/screenshots/all-three-brazilian-recipes.png',
      fullPage: true 
    });
